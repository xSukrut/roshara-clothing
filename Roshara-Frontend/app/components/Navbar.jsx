    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-48 bg-white border rounded shadow text-sm z-50">
          <Link href="/account" className="block px-4 py-2 hover:bg-gray-50" onClick={() => setOpen(false)}>My Account</Link>
          <Link href="/account/orders" className="block px-4 py-2 hover:bg-gray-50" onClick={() => setOpen(false)}>My Orders</Link>
          {(user.role === "admin" || user.isAdmin) && (
            <Link href="/admin" className="block px-4 py-2 hover:bg-gray-50" onClick={() => setOpen(false)}>Admin</Link>
          )}
          <button onClick={() => { setOpen(false); onLogout?.(); }} className="w-full text-left px-4 py-2 hover:bg-gray-50">Logout</button>
        </div>
      )}
    </div>
  );
