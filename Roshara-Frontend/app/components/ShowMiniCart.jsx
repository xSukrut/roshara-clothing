        ...
        >
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex flex-col justify-center items-center text-gray-500 py-8 text-sm">
              ...
              <Link href="/shop" onClick={closeMiniCart} className="mt-4 text-sm font-medium text-white bg-black px-4 py-2 rounded-full hover:bg-gray-900 transition">Continue Shopping</Link>
            </div>
          ) : (
            <>
                  ...
                  return (
                    ...
                    >
                      <div className="relative w-18 h-18 rounded-lg overflow-hidden bg-gray-100 group">
                        <Image src={imgSrc} alt={item.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5 truncate">
                          {item.size && `Size: ${item.size} • `}₹{unitPrice} × {qty}
                        </p>
                      ...
                      <button onClick={() => removeItem(item.product, item.size, item.customSize || null)} className="p-1 rounded-full hover:bg-red-50 transition" title="Remove item">
              ...
              <div className="mt-5 border-t border-gray-200 pt-3 text-sm space-y-1 text-gray-700">
                <div className="flex justify-between"><span>Subtotal</span><span className="font-medium">₹{itemsPrice}</span></div>
                <div className="flex justify-between"><span>Shipping</span><span className="font-medium">{shippingPrice === 0 ? "Free" : `₹${shippingPrice}`}</span></div>
                <div className="flex justify-between"><span>Tax (5%)</span><span className="font-medium">₹{taxPrice ?? 0}</span></div>
                <div className="flex justify-between font-semibold border-t border-gray-200 pt-2 text-base text-gray-900"><span>Total</span><span className="text-amber-700">₹{totalPrice}</span></div>
              </div>

              <Link href="/cart" onClick={closeMiniCart} className="mt-5 block text-center bg-black text-white py-2.5 rounded-xl font-medium hover:bg-gray-900 active:scale-[0.98] transition-all">Go to Checkout</Link>
            </>
          )}
        </motion.div>
      )}
